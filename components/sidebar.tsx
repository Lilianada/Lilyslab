...
const NavItem = ({ href, icon, label, external = false, onClick, delay = 0 }: NavItemProps) => {
  ...
  if (external) {
    return (
      <a
        ...
        className={`flex items-center gap-3 rounded-md px-3 py-2 text-xs text-muted-foreground hover:bg-accent hover:text-foreground transition-all duration-300 ${
  ...
  return (
    <Link
      href={href}
      className={cn(
        "flex items-center gap-3 rounded-md px-3 py-2 text-xs hover:bg-accent transition-all duration-300",
...
export default function Sidebar({ mobile = false, onNavClick }: { mobile?: boolean; onNavClick?: () => void }) {
  ...
  return (
    <aside className={cn("border-r", mobile ? "w-full" : "hidden w-64 md:block")}>
      <div className={cn("p-4", mobile ? "" : "sticky top-0 h-screen flex flex-col")}>
        {!mobile && (
          <div className="mb-8 animate-fade-in">
            <Link href="/" className="flex items-center gap-2">
              <Image src="/logo.png" alt="Lily's Lab Logo" width={40} height={40} className="rounded-md bg-white" />
