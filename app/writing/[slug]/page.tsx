...
export default async function BlogPost({ params }: { params: { slug: string } }) {
  ...
  try {
    ...
    // Format the page ID for the unofficial client (remove dashes)
    const formattedPageId = article.id.replace(/-/g, "")

    // Fetch the page content using the unofficial client
    ...
    return (
      <div className="max-w-3xl mx-auto animate-fade-in px-4 md:px-0">
        ...
        <article>
          ...
          {/* Add the interactions component */}
          <ArticleInteractions slug={params.slug} initialLikes={article.likes || 0} />
        </article>
      </div>
    )
  } catch (error) {
    console.error(`Error rendering article ${params.slug}:`, error)
    return (
